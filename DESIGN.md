...
### Studio (round 6)
The track title carries a visible row under it for Role and Story note (Role's hint defines it: the track's place in the arc). The save bar holds Write next (ghost, phones only), Help, "Save version…" (an inline form) and Save now, and shrinks them to 44px icons with their names kept below 48em; it sticks only when the window is at least 31.3125em tall and the header plus the bar take less than 35% of it. The Studio's Undo pauses while it has focus or the pointer and, if it lapses with focus on it, moves focus to the affected row and says so. The starter-loop hint under a fresh section is Ash Ink guidance, not Ember. Known gap: the docked player's status line (`playerbar.tsx`) still appears together with the bar, as a live region mounted already filled; it should become a LiveStatus mounted with the page.
...
**The Names, Not Codes Rule.** A theme is shown by its name (catalog caps, truncated with the full name on hover and in the accessible name) wherever there is room; single-letter keys, with a legend the table names as its description, only in the narrowest layouts.
