...
**The 320/200 Rule.** Nothing sticks out at 320px with 200% text. Every row that pairs text with a trailing action or badge lets the text keep at least 12rem or stacks the action below it (the catalog row drops its status chip and chevron under the catalog line in a container under 20rem); header actions collapse to 44px icons with their names kept; display titles step down in a narrow container instead of breaking inside words (the catalog row's album title too: `display-card`, 1.125rem, 1.25rem from md, at most 12% of the row with a 1rem floor, so an eleven-letter word such as "Frequencies" stays whole; long words in album titles also get soft hyphens from `@/lib/soft-hyphens`, since `hyphens-auto` gives none without a dictionary); every separator ends the item before it (a word joiner) and never starts a line; every table is in a TableScroller with its scroll cue; and nothing outside a scroller is wider than the column. Track-level titles step down too: the Studio's editable title is capped at 13cqi with a 1rem floor and its number drops to the spine's size below a 14rem row; the share page's Sequence and Discover's lyrics set the number at the spine's size below a 20rem list, and the excerpt drops its indent there. On a touch screen the wizard's track-count slider moves only after a clearly sideways drag (a tap or vertical swipe never changes it; the ± steppers remain). An arrival line that takes focus is not also put in a live region.
...
- **Focus:** the global saffron ring, 2px, offset 2px. No per-component focus styles, with one exception: rows and tabs inside a scroller or a tight list (sidebar rows, album tabs, the folded tab toggle, and the spine's Tracks in sequence region, flush in the side column that scrolls) draw the ring inset (`focus-visible:-outline-offset-2`) so it is never clipped or painted over by the next row. Every element carries the ring's color before it is focused, so the ring appears at once instead of fading in from the text color under `transition-colors`. The global element styles live in the base layer, beneath the utilities, so these adjustments apply.
...
### Studio (round 6)
The track's heading is its title, edited in place (`TrackTitle`, `id="song-title"`, named "Track title"): the number in Ash Ink, then the title in the Headline size, wrapping like a heading (its height follows its text; line breaks become spaces), a Control Rule hairline under it and the focus ring while it is edited; the heading keeps one name ("Track 01: Storm Warning"). Under it only when needed: the empty-title error in Coral, or Stone Ink guidance when another track has the same title, trimmed and in any casing ("Track 03 is also called this.": a reprise can be deliberate, so it is said, not flagged). A row for Role and Story note follows (Role's hint defines it: the track's place in the arc). Below 42rem it folds, with the track's album-theme toggles, into one line under the title: "Role and Story note", Edit/Hide, and what they hold ("Opening · She leaves the lighthouse before the… · Carries tide", or "No role yet · no Story note yet"); it starts closed, and `focus=role` and `focus=story` open it. From 42rem until the track list has theme columns (64rem), the track's album-theme toggles sit in Track details after Key and Tempo, never between the title and the lyrics, so the lyrics keep the first screen at 1280×800 with themes too. Below 42rem Preview song and More are 44px icons (names kept) and the title needs 12rem, so on a phone they share the title's row. Where the editor is under 36rem the Sections list becomes 44px chips set in one line of text with the heading and Add section (an icon), wrapping like words; a written section shows a check, and the rows' status line stays in each chip's accessible name. Key and Tempo sit in "Track details" after Sections; the editable heading adds no height, so the lyrics editor still reaches the first screen on a laptop (1440×900 and 1280×800). A track moves one place from More (Move track up / down; Ctrl+Alt+Shift+PageUp/PageDown anywhere, or with ↑/↓ outside text fields; listed by the save bar's quiet "Shortcuts" button and on Help) or any distance in one step (Move to position…: an inline form under the track header like the delete question, a select of every place such as "05 · now “Signal”", Move and Cancel; focus on the select, and Escape, Cancel or Move return it to More). Every move is announced and shown with Undo ("Moved “Track 1” (now “Track 2”) from 01 to 02."); moves of one track in a row share one Undo, which puts it back where the run started with every default name as it was (`moveTrackTo`, `undoTrackMove`). Undoing a delete says so ("Put back “Track 3”.", "Put back Verse 2."). Beside the editor, the track list's scroller has a scroll-padding-top equal to its sticky column heads, so a row reached by Tab or Shift+Tab lands clear of them. The save bar is one row at every width: Help, "Save version…" (an inline form) and Save now, as one group that never wraps apart; Write next is the screen's primary and follows the chords in the editor at every width (never sticky); below 48em the bar's buttons show short names ("Help", "Version", "Save", accessible names unchanged), Help gives way on a touch phone (it stays in the app navigation), and below 22em they become 44px icons with their names kept; it sticks only when the window is at least 31.3125em tall and the header, the bar and the docked preview player together take less than 35% of it. Arriving at the lyrics ("Write track N", Write next) brings the track header just under the save bar with the lyrics below it when both fit, otherwise the lyrics alone; the page's scroll padding places it, never a second offset. Below 42rem "Skip to the lyrics" is a visible ink link at the top of the Studio; from 42rem it is hidden and the app's skip link (the page's first stop) does its work. Track details stacks Key and Tempo below a 14.5rem container, so the select always shows its whole key, and the track's catalog line uses CatalogItems (a separator never starts a line). The track list, story editor, album details, AI draft panel and comments are memoized with stable handlers (`useStableEvent`), so a keystroke re-renders only the editor. The save bar's quiet "Shortcuts" button (a disclosure, from a 23rem status box, fine pointers only) opens a small list of the four chords over the page, ending on Help; Escape, a press outside or focus leaving closes it. Undo, and Retry after a failed save, lie over the save status and the Shortcuts button (which hides but keeps its place while one shows) in a box that is always at least 44px tall, so the bar never reflows when one appears or lapses. The sentence is never clamped, so a name is never cut; below 22em the offer covers the bar's first row with the actions hidden beneath it, and a third line on a narrow bar grows the overlay down over the page. Adding, deleting or moving a track or section saves at once, and a save goes with keepalive so a reload a moment later keeps the change: a routine save up to 32 KB keeps room for a last-chance save beside it in the browser's 64 KiB keepalive budget, a larger album's save takes keepalive whenever the budget is free, and the last-chance save on leaving may use the whole budget (`KeepaliveBudget`). Saving a version says which ("Saved “First pass” as a version."); the Overview's checklist disclosure reads "Show all 7 steps" closed and "Hide the steps" open (aria-expanded). The Studio's Undo pauses while it has focus or the pointer and, if it lapses with focus on it, moves focus to the affected row and says so. The starter-loop hint under a fresh section is Ash Ink guidance, not Ember. Below a 42rem container (the single-column layout) the Studio's list, headed "Sequence" like the spine, folds into a disclosure ("Sequence · 04 of 10 · Track 4", closed at first, remembered for the session; each separator is held to the item before it by a no-break space, below a 12rem column the summary steps down to 1rem and drops the title ("Sequence · 01 of 02"), and the heading asks for 12rem before Add track sits beside it), so the current track's editor comes straight after the header. A track whose title is still its default ("Track 5") is renamed when its position changes, so a number and its name always agree; written titles are never touched. A daily challenge opened from Challenges (`?challenge=<key>`) pins a quiet band directly above the Lyrics draft (`ChallengeBand`): the title and prompt, and a row always 44px tall with "Claim N credits" (secondary) once the track has lyrics written today, else the line saying so; the claim saves first and checks lyrics written since the day's baseline, and Hide returns focus to the lyrics. Sections move with Undo like tracks (one Undo per run, focus back on the section row); announcements put titles (the artist's words) in curly quotes and section labels bare ("Moved Verse 1 to section 2 of 2.", "Deleted Verse 2."). Deleting a track or section that holds written lyrics asks first, inline (a section's question sits under Add section in the section list, never above the lyrics) ("Delete Track 1 and its 2 written sections?"; focus on Delete, Escape or Cancel back to More), and keeps its Undo after; an empty or starter-only one deletes at once with Undo. Add section moves focus to the new section's lyrics and announces it ("Verse 2 added, section 3 of 3"). The section's Preview is a ghost button named for the section ("Preview Verse 1") and its More is always the 44px ⋯ icon, so it never reads like the track's outlined "Preview song · More". Choosing a section announces it like stepping to it; a delete announces "Deleted …" once (Undo's description is left out on that first focus); a move of a default-named track is said by place ("Moved track 3 to track 2 of 10, now called “Track 2”."). The docked player announces only what it changes itself (an instrument notice, instrument loading), into one LiveStatus that `playerbar.tsx` mounts with the Studio page, empty, before the bar docks. The bar's visible status line is plain text, and a preview's loading and result are announced by the Studio's own preview status line, not twice.
...
### Named Rules
**The Wizard Rule.** One mark for a field that can be left empty: "Optional" in Ash Ink after its label; required fields carry none. The narrow step tabs show only "Now" or a check (the words stay for screen readers). A step change focuses the step's heading and scrolls the steps and heading into view. The References field takes "Title — Artist", one per line (a dash, " by " after two words, or a comma also work, `@/lib/reference-line`), and the preview shows the split before saving.

**The Landing Focus Rule.** A line that takes focus on arrival (tabindex -1, never a Tab stop: the welcome line, an arrival status) shows no ring (`data-focus-landing`), and a new page starts scrolled to its top.

**The Honest Signal Rule.** A tick, a score, a fraction or a status never claims work that wasn't done. Placeholder lyrics and the starter chord loop count as not written everywhere they are counted; `@/lib/lyrics` and `@/lib/chords` are the only definitions. A dimension score can't exceed its own evidence: Harmony never passes the share of tracks with chords of their own, and Motifs stays at 40 or below with no motifs and 60 or below while none returns on a second track (once the album has a second track).

**The One Term Rule.** One word per concept on every surface (labels, headings, fix links, spine heads, search, page titles): "Story note" for a track's narrative summary, "Role" for its place in the arc, "Motifs" for album motifs plus track motif tags, read from the same source on the Story bible and the Coherence report; "Story bible" for what the album is about (its tab, heading and page title; never "Album Bible" or a bare "Bible", since there are two) and "Sound bible" for how it sounds (never "Style bible", "Voice / style bible" or "Voice and style"), each linking to the other in its heading row ("How it sounds → Sound bible", "What it's about → Story bible"); "Discover" for the public feed, its h1 included (never "Community albums"); "References". Sound bible completeness reads "3 of 9 fields set" everywhere (the Sound bible form, the Overview row, the readiness and publish list, the Story bible summary), from `@/lib/sound-bible-progress` `soundBibleFieldsSet()`; never "sections" (a Section is part of a song) or "parts". CONTEXT.md holds the glossary. "Sequence" is the spine; the Coherence dimension for track order and song structure is "Flow" (key `sequence`). The order of the tracks is "the sequence" in all copy, never "tracklist" or "track list"; the create wizard's third step is "Sequence".
...
**The Album Frame On A Phone.** The catalog line's status reads "On Discover" for an album on Discover (never "Published · On Discover"), and the edited time is left out below a 30rem header. The frame's first stop is a skip link named for the page ("Skip to the Coherence report"), landing on `#album-page`: a visible ink link above the title below a 42rem header, the focus-only chip from 42rem, and absent on the Studio (which has "Skip to the lyrics"). Below 42rem the header's gaps tighten; under a 12rem header the release title's size and floor are both 12cqi, so a twelve-letter word stays whole. Library and Home chips say "On Discover" too. A remix notification links to the remix on Discover while it is published (the notification records `remixAlbumId`), otherwise to the owner's album with a line saying why; notification titles carry the Strong Rule underline at rest. On Coherence, while any track is unwritten, one neutral line under the scores says low early numbers are progress, not a fault, and links to Help's "What counts as written". The app's skip link is the page's one skip link, the first stop, named for the page ("Skip to the Overview", "Skip to the lyrics"); the album frame's and the Studio's own links are only the visible touch links below a 42rem header.

**The Comments Rule.** Comments and tasks name a section as the spine does ("01 · Low Tide Leaving · Verse 1", `@/lib/section-place`, found by section id wherever its track now sits). Actions are labelled buttons under the comment; Delete asks inline like the Studio's delete question; Create task is busy, then "Task created" (a link to Comments and tasks), never twice (the server refuses a second task for one comment). The Export tab opens on the zip: what it holds, its button and the balance above the format choices, with the free handoff packs after it; each handoff confirmation ends on its next step. A comment with an open task is that task: listed and counted once (under Open tasks, "From <author>’s comment"), it reads "Tracked as a task" (a link) with no Resolve; Mark done resolves it, resolving the comment marks the task done, and reopening either reopens the other (`@/server/comment-tasks`).

**The One Matrix Per Page Rule.** The Theme map's Theme column always sticks: 12rem or 40% of the scroller, 5rem below a 28rem container and 3.5rem below 14rem, where names are catalog caps on up to two lines (`themeHeadLines`) over their count and a list under the table gives each theme's full name and arc. On the Story bible the spine leaves out its theme columns (number, title, Lyrics and Role only; `AlbumSpine showThemes={false}`, chosen from the address by `AlbumFrameBody`) and its side column keeps its narrowest width: the Theme map is that page's one view of tracks against themes. On Discover, a list row is a size container: from 40rem Like sits at the row's end, below it Like and its count drop under the catalog line so the title and concept take the whole width; the lyric strip is captioned "Lyrics", one bar per track (a full ink bar for written lyrics, a low stub for none), never the spine's ThemeMark, which only ever means a theme carried. Below a 40rem form, Sort, Show and Genre fold into a "Filters" disclosure whose summary names the sort and any narrowing filter ("Filters · Newest · Finished only"), open by default when a filter is set; from 40rem the selects always show. Home's Continue card tells the One Score Story under its catalog line (`ProgressLine`, shared with the Library rows), and its status reads "On Discover" for a published album. On the Discover list, under the lyric strip, each of the album's themes carries its thread through the sequence: one mark per track on the lyric strip's 8px pitch, a 4px Bone Ink square where the track carries the theme and a Strong Rule dot where it doesn't (ThemeMark's meaning at strip size), read as one sentence ("Themes: isolation on 6 of 8 tracks, signal on every track."); before any track is tagged the names stand alone.
...
**The Warn For Problems Rule.** Ember and Coral are for things that are actually wrong: a contradiction, an error, a balance that can't cover a spend. Ordinary guidance (an unwritten track, a starter loop, "not written yet", "Unfinished") is Stone or Ash Ink, and its chip is neutral ("Start here", "To do"). The setup's defaults are never a warning: while every tempo is still the setup's 120 BPM or unset, it is a "To do" ("Set each track's tempo", naming the tracks); "section energy repeats" is kept for repetition the artist set. While tracks are unwritten, the Story bible's threads are "Threads to pick up", written as next steps with neutral To do chips, after a line naming what is written; before a first score, the Coherence report opens on what is done, then says "For a first score:" once.
