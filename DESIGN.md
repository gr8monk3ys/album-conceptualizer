...
The printed Album Bible (`server/bible-pdf.ts`) uses its own `PRINT` palette of near-black inks for white paper, not the UI tokens.
...
### Coherence report
While any track is unwritten the report leads with progress, not with a verdict: "4 of 8 tracks written · Unfinished", then the written tracks' own score as the large figure ("72/100 · The written tracks score 72", `CoherenceReport.writtenScore`, the same weighting over each dimension's uncapped score), and only then the whole album's capped score in Stone Ink ("The whole album scores 45/100 for now: no dimension counts above 50 until the other 4 tracks have lyrics"). "Unfinished" is neutral ink, not Ember. Findings that are work not done yet (`CoherenceIssue.progress`: unwritten lyrics, chords, keys, story notes, themes, motifs, a concept summary, thin structure) wear a neutral chip: "Start here" on the unwritten tracks, pinned first, and "To do" on the rest; "Fix first" (Coral) and "Warning" (Ember) are kept for real contradictions such as duplicate track numbers, theme drift or motifs that never return. "How this is scored" ends with a link to Help's "What counts as written". The Overview's Coherence row reads the same way ("Unfinished · 3 of 7 tracks written · written tracks 72/100").
...
### Tag from lyrics
The Bible's "Tag from lyrics" proposes; the artist ticks and adds. After "Add N tags" the status names what was added ("Added tide to 04, signal and static to 05.") and an Undo sits beside it for at least 10 seconds, timed by the Studio's `useUndoWindow`: the clock stops while the Undo has focus or the pointer, a full window starts again when both leave, and if it lapses with focus on it, focus goes to "Tag from lyrics" and the line adds "Undo has ended; the tags stay." Undo takes off exactly the tags that apply added (`removeAddedTags`, `POST /api/albums/<id>/autotag/undo`), never tags added by hand since, and says so ("Undone: took tide off 04."). Proposed phrases keep the lyric's word order: a phrase never spans punctuation, and a tie is spelt the way the lyrics say it first ("last ferry", never "ferry last").

### Studio (round 6)
The track title carries a visible row under it for Role and Story note (Role's hint defines it: the track's place in the arc). Title, Key and Tempo sit in a "Track details" section after Sections, so the lyrics editor reaches the first screen on a laptop (1440×900 and 1280×800). The save bar holds Write next (the primary, below 48em only; from 48em it sits in the editor), Help, "Save version…" (an inline form) and Save now, and shrinks them to 44px icons with their names kept below 48em; it sticks only when the window is at least 31.3125em tall and the header plus the bar take less than 35% of it. Undo, and Retry after a failed save, lie over the save status in a column that is always at least 44px tall, so the bar never reflows when one appears or lapses. The Studio's Undo pauses while it has focus or the pointer and, if it lapses with focus on it, moves focus to the affected row and says so. The starter-loop hint under a fresh section is Ash Ink guidance, not Ember. The docked player announces only what it changes itself (an instrument notice, instrument loading), into one LiveStatus that `playerbar.tsx` mounts with the Studio page, empty, before the bar docks. The bar's visible status line is plain text, and a preview's loading and result are announced by the Studio's own preview status line, not twice.
