...
async def export_progression_mp3(
    ...
    """
    Render chord progression to an MP3 preview.

    This is optional and requires:
    - `fluidsynth` + `ffmpeg` in PATH
    - a SoundFont (.sf2) configured via `AC_SOUNDFONT_PATH` (or `SOUNDFONT_PATH`)
    """
    try:
        from album_conceptualizer.export.midi import create_chord_midi
        from album_conceptualizer.models.music_theory import Chord
    ...
    try:
        ...
        raise HTTPException(status_code=501, detail="Audio rendering not available.") from None

    soundfont_path = get_soundfont_path()
    if not soundfont_path:
        raise HTTPException(
            status_code=501,
            detail="MP3 rendering requires AC_SOUNDFONT_PATH pointing to a .sf2 file.",
        )
    ...
    with tempfile.NamedTemporaryFile(suffix=".mid", delete=False) as f_midi:
        midi.write(f_midi.name)
        midi_path = f_midi.name
    ...
    try:
        ...
        detail = str(exc)
        status_code = 501 if "not found" in detail.lower() else 502
        raise HTTPException(status_code=status_code, detail=detail) from None

    filename = data.title or "progression"
    filename = filename.replace(" ", "_").lower()

    background_tasks.add_task(_cleanup_temp_file, midi_path)
    background_tasks.add_task(_cleanup_temp_file, mp3_path)
    response = FileResponse(
        path=mp3_path,
