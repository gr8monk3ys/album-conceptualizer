...
class ChordProgressionSearchInput(BaseModel):
    ...
    mood: str | None = Field(default=None, description="Filter by mood/emotion")
    top_k: int = Field(default=5, description="Number of results to return")
...
class NarrativeStructureSearchInput(BaseModel):
    ...
    query: str = Field(
        description="Search query for narrative structures or concept album analyses"
    )
...
class MotifTrackerInput(BaseModel):
    ...
    track_number: int | None = Field(default=None, description="Track number for tracking")
    section: str | None = Field(default=None, description="Section where motif appears")
    variation: str | None = Field(default=None, description="Variation notes")
