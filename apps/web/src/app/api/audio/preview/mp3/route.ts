...
export async function POST(request: Request) {
  ...
  const rate = await checkRateLimit("preview_audio", `user:${userId}`);
  const rateFailure = getRateLimitFailure(rate, "Too many renders. Please wait a bit and try again.");
  if (rateFailure) {
  ...
  const engineResponse = await engineFetch("/export/progression/mp3", {
    method: "POST",
