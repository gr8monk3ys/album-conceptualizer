...
export function AlbumRoughDemoWorkspace({
  ...
  async function saveDemo() {
    ...
    try {
      ...
      const payload = (await response.json().catch(() => null)) as
        ...
        | { error?: string }
        | null;
      if (!response.ok || !payload || !("demo" in payload)) {
        throw new Error(payload && "error" in payload && payload.error ? payload.error : "Save failed.");
      }

      setDemos((current) => sortDemos([payload.demo, ...current.filter((demo) => demo.id !== payload.demo.id)]));
  ...
  async function deleteDemo(demoId: string) {
    ...
    try {
      ...
      setDemos((current) => current.filter((demo) => demo.id !== demoId));
      if (editingId === demoId) {
        setEditingId(null);
        setForm(emptyForm());
      }
